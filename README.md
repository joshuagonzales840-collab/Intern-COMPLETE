...
The public site needs the Firebase HTTPS function in `functions/` for payment requests. Static hosting alone returns `405 Method Not Allowed` for checkout.

From this folder, install and sign in to Firebase CLI, then deploy the function and hosting rewrite:

```powershell
npm install -g firebase-tools
firebase login
firebase use intern4-27de2
firebase functions:secrets:set PAYMONGO_SECRET_KEY
firebase deploy --only functions:paymongoApi,hosting
```

When prompted for the secret, enter the PayMongo secret key directly in the terminal. Do not put it in HTML or JavaScript. The domain must be the Firebase Hosting site for project `intern4-27de2`, or its custom domain connected to that site.
